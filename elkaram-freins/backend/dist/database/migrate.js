...
async function migrate(pool) {
    ...
    `);
        ...
        try {
            await conn.execute("ALTER TABLE documents ADD COLUMN matricule VARCHAR(255) DEFAULT '' AFTER supplier_id");
        }
        catch { /* column may already exist */ }
        try {
            await conn.execute("ALTER TABLE company_settings ADD COLUMN logo_base64 LONGTEXT AFTER logo_path");
        }
        catch { /* column may already exist */ }
        try {
            await conn.execute("ALTER TABLE suppliers ADD COLUMN ice VARCHAR(255) AFTER fiscal_id");
        }
        catch { /* column may already exist */ }
        // Payments table
        await conn.execute(`
    ...
    `);
        ...
        if (userRows[0].count === 0) {
            ...
            await conn.execute('INSERT IGNORE INTO users (id, username, email, password, full_name, role) VALUES (?, ?, ?, ?, ?, ?)', ['u-admin', 'admin', 'admin@elkaram.dz', hashedPassword, 'Administrateur', 'admin']);
        }
        // Seed default designs
        const [designRows] = await conn.execute('SELECT COUNT(*) as count FROM document_designs');
        if (designRows[0].count === 0) {
            ...
            for (const d of designs) {
                await conn.execute('INSERT IGNORE INTO document_designs (name, primary_color, secondary_color, font_family, header_style, is_default) VALUES (?, ?, ?, ?, ?, ?)', d);
            }
        }
        // Seed default company settings
        const [settingsRows] = await conn.execute('SELECT COUNT(*) as count FROM company_settings');
        if (settingsRows[0].count === 0) {
            await conn.execute('INSERT IGNORE INTO company_settings (id, company_name, currency, tax_rate) VALUES (?, ?, ?, ?)', [1, 'EL Karam Freins SARL', 'MAD', 20.0]);
        }
        // Seed default document columns
        const [colRows] = await conn.execute('SELECT COUNT(*) as count FROM document_columns');
        if (colRows[0].count === 0) {
            ...
            for (const dt of docTypes) {
                for (const col of columns) {
                    await conn.execute('INSERT IGNORE INTO document_columns (document_type, column_name, visible) VALUES (?, ?, ?)', [dt, col, 1]);
                }
            }
        }
    }
