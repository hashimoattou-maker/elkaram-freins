...
router.post('/:id/payment', (0, auth_1.requireRole)('admin', 'user'), async (req, res) => {
    ...
    try {
        ...
        await conn.execute('INSERT INTO payments (id, document_id, amount, payment_method, payment_account, payment_date, note, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [paymentId, id, amount, paymentMethod || 'espèces', paymentAccount || '', paymentDate || new Date().toISOString().slice(0, 19).replace('T', ' '), note || '', req.user.id]);
        ...
        await conn.commit();
        await database_1.default.execute('INSERT INTO audit_log (user_id, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?)', [req.user.id, 'payment', 'document', id, `Paiement de ${amount} MAD sur ${doc.doc_number}`]);
...
router.get('/:id/payments', (0, auth_1.requireRole)('admin', 'user'), async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await database_1.default.execute('SELECT * FROM payments WHERE document_id = ? ORDER BY created_at DESC', [id]);
        res.json(rows.map((r) => ({
            id: r.id,
...
router.delete('/:docId/payments/:paymentId', (0, auth_1.requireRole)('admin', 'user'), async (req, res) => {
    const conn = await database_1.default.getConnection();
    try {
        const { docId, paymentId } = req.params;
        const [payRows] = await conn.execute('SELECT * FROM payments WHERE id = ? AND document_id = ?', [paymentId, docId]);
        if (payRows.length === 0) {
            res.status(404).json({ error: 'Paiement non trouvé' });
            return;
        }
        const payment = payRows[0];
        const [docRows] = await conn.execute('SELECT * FROM documents WHERE id = ?', [docId]);
        if (docRows.length === 0) {
            res.status(404).json({ error: 'Document non trouvé' });
            return;
        }
