    ...
    try {
        ...
        for (const row of data) {
            try {
                ...
                const [existsRows] = await conn.execute('SELECT id FROM clients WHERE code = ?', [row.code]);
                if (existsRows.length > 0) {
                    errors++;
                    continue;
                }
                await conn.execute(`INSERT INTO clients (id, code, name, company, address, phone, email, fiscal_id, ice, commercial_id, article_id, credit_limit, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [(0, helpers_1.generateId)(), row.code, row.name, row.company || '', row.address || '', row.phone || '', row.email || '', row.fiscal_id || '', row.ice || '', row.commercial_id || '', row.article_id || '', row.credit_limit || 0, row.notes || '']);
                imported++;
            }
            catch {
                errors++;
