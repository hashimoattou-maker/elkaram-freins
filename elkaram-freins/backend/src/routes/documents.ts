...
async function getNextDocNumber(type: string): Promise<string> {
  ...
  const [rows] = await pool.execute(
    "SELECT doc_number FROM documents WHERE doc_type = ? AND doc_number LIKE ? ORDER BY doc_number DESC LIMIT 1",
    [type, `${prefix}-${year}-%`]
  ) as any;
...
async function updateStockForDocument(doc: any, lines: any[], movementType: string): Promise<void> {
  for (const line of lines) {
    if (!line.product_id) continue;

    const [productRows] = await pool.execute('SELECT stock FROM products WHERE id = ?', [line.product_id]) as any;
    if (productRows.length === 0) continue;

    const qty = movementType === 'sortie' ? -line.quantity : line.quantity;
    await pool.execute("UPDATE products SET stock = stock + ?, updated_at = NOW() WHERE id = ?", [qty, line.product_id]);

    await pool.execute(
      `INSERT INTO stock_movements (id, product_id, document_id, movement_type, quantity, unit_price, reference, notes, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [generateId(), line.product_id, doc.id, movementType === 'entrée' ? 'entrée' : 'sortie',
...
function mapLineRequest(l: any): any {
  return {
...
router.get('/', async (req: AuthRequest, res: Response) => {
  try {
    ...
    let where = 'WHERE 1=1';
    const params: any[] = [];
    const docTypeFilter = (doc_type || docType) as string | undefined;
    const clientIdFilter = (client_id || clientId) as string | undefined;
    const supplierIdFilter = (supplier_id || supplierId) as string | undefined;

    if (docTypeFilter) { where += ' AND d.doc_type = ?'; params.push(docTypeFilter); }
    if (status) { where += ' AND d.status = ?'; params.push(status); }
    if (clientIdFilter) { where += ' AND d.client_id = ?'; params.push(clientIdFilter); }
    if (supplierIdFilter) { where += ' AND d.supplier_id = ?'; params.push(supplierIdFilter); }
    if (date_from) { where += ' AND d.date >= ?'; params.push(date_from); }
    if (date_to) { where += ' AND d.date <= ?'; params.push(date_to); }

    const [countRows] = await pool.execute(`SELECT COUNT(*) as count FROM documents d ${where}`, params) as any;
...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [companyRows] = await pool.execute('SELECT * FROM company_settings WHERE id = 1') as any;
...
router.post('/', requireRole('admin', 'user'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const computed = calculateTotals(lines);
    ...
    const [docLines] = await pool.execute('SELECT * FROM document_lines WHERE document_id = ? ORDER BY line_number', [id]) as any;

    await pool.execute('INSERT INTO audit_log (user_id, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?)',
      [req.user!.id, 'create', 'document', id, `Création document ${docNumber}`]);
...
router.put('/:id', requireRole('admin', 'user'), async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const [existingRows] = await conn.execute('SELECT id, status FROM documents WHERE id = ?', [id]) as any;
    ...
    if (rawLines && Array.isArray(rawLines) && rawLines.length > 0) {
      ...
      params.push(id);
      await pool.execute(`UPDATE documents SET ${updates.join(', ')} WHERE id = ?`, params);
    }
...
router.delete('/:id', requireRole('admin', 'user'), async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const [existingRows] = await pool.execute('SELECT id, status FROM documents WHERE id = ?', [id]) as any;
    if (existingRows.length === 0) {
      res.status(404).json({ error: 'Document non trouvé' });
      return;
    }
    ...
    await pool.execute("UPDATE documents SET status = 'annulé', updated_at = NOW() WHERE id = ?", [id]);

    await pool.execute('INSERT INTO audit_log (user_id, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?)',
      [req.user!.id, 'cancel', 'document', id, 'Annulation de document']);
...
router.post('/:id/validate', requireRole('admin', 'user'), async (req: AuthRequest, res: Response) => {
  const conn = await pool.getConnection();
  try {
    const { id } = req.params;
    ...
    await pool.execute('INSERT INTO audit_log (user_id, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?)',
      [req.user!.id, 'validate', 'document', id, `Validation document ${doc.doc_number}`]);
...
router.post('/:id/convert', requireRole('admin', 'user'), async (req: AuthRequest, res: Response) => {
  const conn = await pool.getConnection();
  try {
    const { id } = req.params;
    ...
    const [sourceDocRows] = await conn.execute('SELECT * FROM documents WHERE id = ?', [id]) as any;
    ...
    const [sourceLines] = await conn.execute('SELECT * FROM document_lines WHERE document_id = ? ORDER BY line_number', [id]) as any;

    const newDocNumber = await getNextDocNumber(targetTypeValue);
    const newId = generateId();

    const computed = calculateTotals(sourceLines as any);
    ...
    for (let index = 0; index < sourceLines.length; index++) {
      const line = sourceLines[index] as any;
      await conn.execute(`
        INSERT INTO document_lines (id, document_id, product_id, line_number, description, quantity, unit, unit_price, tax_rate, discount_percent, total_ht, total_ttc)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [generateId(), newId, line.product_id, index + 1, line.description,
    ...
    await pool.execute('INSERT INTO audit_log (user_id, action, entity_type, entity_id, details) VALUES (?, ?, ?, ?, ?)',
      [req.user!.id, 'convert', 'document', id, `Conversion de ${sourceDoc.doc_number} vers ${newDocNumber}`]);

    const [newDocRows] = await pool.execute('SELECT * FROM documents WHERE id = ?', [newId]) as any;
    const [newLines] = await pool.execute('SELECT * FROM document_lines WHERE document_id = ? ORDER BY line_number', [newId]) as any;
...
router.get('/:id/pdf', async (req: AuthRequest, res: Response) => {
  try {
    ...
    let design = null;
    if (doc.design_id) {
      const [designRows] = await pool.execute('SELECT * FROM document_designs WHERE id = ?', [doc.design_id]) as any;
