  ...
  try {
    ...
    for (const row of data) {
      try {
        ...
        const [existsRows] = await conn.execute('SELECT id FROM suppliers WHERE code = ?', [row.code]) as any;
        ...
        await conn.execute(
          ...
          [generateId(), row.code, row.name, row.company || '', row.address || '', row.phone || '', row.email || '', row.fiscal_id || '', row.ice || '', row.notes || '']
        );
        imported++;
      } catch { errors++; }
    }
