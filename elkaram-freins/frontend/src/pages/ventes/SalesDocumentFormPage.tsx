...
import { ArrowLeft, Save, CheckCircle, Plus, Trash2, CreditCard } from "lucide-react";
...
export default function SalesDocumentFormPage() {
  ...
  const docType = type ? DOC_TYPE_MAP[type] : "devis";
  const isEdit = !!id;
  ...
  useEffect(() => {
    clientsApi.getClients({ limit: 100 }).then((r) => setClients(r.data)).catch(() => {});
  ...
  const discountValue =
    form.discountType === "percentage" ? subtotal * (Number(form.discount) / 100) : Number(form.discount);
